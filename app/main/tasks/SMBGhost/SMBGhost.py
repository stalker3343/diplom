...
class VulnerabilityInfo:
    """
    Information about a server's vulnerability to SMBGhost
    """
    def __init__(self):
...
def is_vulnerable(tree: TreeConnect) -> (bool, int, int):
    """
    Checks if an SMB share is vulnerable to SMBGhost
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Test if a Samba server if vulnerable to SMBGhost")
...
def CVE_2021_44142(target, gradation):
    #setup_logging()
    if gradation:
    ...
    try:
        connection.connect((str(target),  445))
